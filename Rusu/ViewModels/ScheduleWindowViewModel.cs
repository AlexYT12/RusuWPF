...
namespace Rusu.ViewModels
{
    public sealed class ScheduleWindowViewModel : ObservableObject
    {
            ...
            CopyButton = new RelayCommand(x =>
            {
                if (Days != null)
                {
                    ...
                    foreach (Day day in Days)
                        text += StringFormater.DayAsString(day, Data.DayTemplatePath, day.ToString()) + Environment.NewLine + Environment.NewLine;
        ...
        {
            if (Date.HasValue) Days = await Parser.ScheduleAsync(Date.Value);
        }
